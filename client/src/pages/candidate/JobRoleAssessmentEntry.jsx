...
const JobRoleAssessmentEntry = () => {
  ...
  useEffect(() => {
    const fetchJobInfo = async () => {
      try {
        const res = await axios.get(getApiUrl(`/public/job-test/${jobToken}`));
        setJob(res.data);
      } catch (err) {
        setError(err.response?.data?.message || 'Invalid or inactive job assessment link.');
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleStart = async (e) => {
    ...
    if (job?.totalQuestions === 0) {
      setError(`No questions available for "${job?.name}". Please contact administrator.`);
      return;
    }
    ...
    try {
      const res = await axios.post(getApiUrl(`/public/job-test/${jobToken}/start`), {
        name: name.trim(),
        email: email.trim(),
        phone: phone.trim()
      });
      ...
      setError(err.response?.data?.message || 'Failed to start assessment session');
      setSubmitting(false);
    }
  };
    ...
    return (
      <div className="min-h-screen bg-[#fff8f7] flex items-center justify-center p-4">
        <div className="bg-white border border-[#e2bfb9] rounded-md p-8 max-w-md w-full text-center space-y-4 shadow-lg">
          ...
          />
          <h2 className="text-xl font-bold text-[#311213]">Assessment Unavailable</h2>
  ...
  return (
    <div className="min-h-screen bg-[#fff8f7] flex items-center justify-center p-4 select-none font-sans text-[#311213]">
      <div className="w-full max-w-xl bg-white border border-[#e2bfb9] rounded-md shadow-lg overflow-hidden space-y-6">
        ...
        <form onSubmit={handleStart} className="p-8 pt-0 space-y-6">
          {error && (
...
export default JobRoleAssessmentEntry;
